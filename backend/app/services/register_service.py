...
from app.services.exceptions import UserAlreadyExistsError, InvalidVerificationCodeError, CodeNotFoundError
...
from app.core.security import hash_verification_code, generate_verification_code, verify_verification_code
...
async def register_user(db, payload: schemas.UserRegistation):
    ...
    if code_hash_db is None:
        raise CodeNotFoundError("Verification code not found for this email")
    
    if not verify_verification_code(payload.code, code_hash_db):
